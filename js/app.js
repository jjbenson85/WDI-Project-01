...
class GameLevel{
  ...
  addTile(tile, player, sound=true){
    ...
    //Remove opponent class and valid from this tile
    $(tile)
  ...
  resetVariables(){
    ...
    const levelDesign = levelDesignArray[level]
  ...
  calculateScores(){
    ...
    this.$hexArray.each(function(){
      $(this).hasClass('white') && whiteCount++
      $(this).hasClass('black') && blackCount++
    })
  ...
  specialCapture(tile){
    if($(tile).hasClass('invert')){
      ...
      $(tile).addClass(player).removeClass(opponent)
    }
